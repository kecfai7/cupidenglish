...
export function TtsSettingsBar({ className = '' }) {
  ...
  return (
    <div className={`flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-pink-300 dark:border-slate-800 px-6 py-4 rounded-3xl text-xs font-black shadow-xl transition-all ${className}`}>
      {/* Title/Header icon */}
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-slate-950 dark:text-white font-black text-xs sm:text-sm">목소리:</span>
        <select
          value={settings.gender}
          onChange={(e) => handleChange('gender', e.target.value)}
          className="bg-pink-50 dark:bg-slate-950 border border-pink-300 dark:border-slate-700 rounded-xl px-3.5 py-1.5 text-xs sm:text-sm font-black text-pink-950 dark:text-white cursor-pointer shadow-sm hover:border-pink-500 focus:outline-none transition-all"
        >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-slate-950 dark:text-white font-black text-xs sm:text-sm">연속 반복:</span>
        <select
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-slate-950 dark:text-white font-black text-xs sm:text-sm">속도:</span>
        <select
